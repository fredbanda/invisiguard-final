...
const MINFRAUD_FACTORS_URL = "https://minfraud.maxmind.com/minfraud/v2.0/factors";
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`Account ID present: ${!!accountId}, License Key present: ${!!licenseKey}`);

    if (!accountId || !licenseKey) {
      console.error("MaxMind API credentials missing");
      return NextResponse.json(
        { error: "MaxMind credentials not configured", code: "CREDENTIALS_MISSING" },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      const rawBody = await request.text().catch(() => "Unable to read request body");
    ...
    try {
          ...
          return NextResponse.json(
            {
              ...
              code: "HTML_RESPONSE",
              htmlPreview: responseText.substring(0, 200) + (responseText.length > 200 ? "..." : ""),
            },
            { status: 502 }
          );
        }
      ...
      try {
        ...
        const rawResponse = await response.text().catch(() => "Unable to read response body");
          ...
          return NextResponse.json(
            {
              error: "Received HTML instead of JSON from MaxMind API",
              code: "HTML_RESPONSE",
              htmlPreview: rawResponse.substring(0, 200) + (rawResponse.length > 200 ? "..." : ""),
            },
            { status: 502 }
          );
        }
...
function transformFormDataToMinFraudFormat(formData: MinFraudRequest): any {
  ...
  if (formData.username || formData.accountCreatedAt) {
    ...
    if (formData.accountCreatedAt) {
      try {
        ...
        console.warn("Invalid date format for accountCreatedAt:", formData.accountCreatedAt);
      }
    }
  }
  ...
  // Billing data
  if (formData.billingFirstName || formData.billingLastName || formData.billingAddress1 || 
      formData.billingCity || formData.billingRegion || formData.billingPostal || 
      formData.billingCountry || formData.billingPhone) {
    
    result.billing = {};
    if (formData.billingFirstName) result.billing.first_name = formData.billingFirstName;
    if (formData.billingLastName) result.billing.last_name = formData.billingLastName;
    if (formData.billingAddress1) result.billing.address = formData.billingAddress1;
    if (formData.billingAddress2) result.billing.address_2 = formData.billingAddress2;
    ...
    if (formData.billingCountry) result.billing.country = formData.billingCountry;
    if (formData.billingPhone) result.billing.phone_number = formData.billingPhone;
  }
  
  // Shipping data
  if (formData.shippingFirstName || formData.shippingLastName || formData.shippingAddress1 || 
      formData.shippingCity || formData.shippingRegion || formData.shippingPostal || 
      formData.shippingCountry) {
    
    result.shipping = {};
    if (formData.shippingFirstName) result.shipping.first_name = formData.shippingFirstName;
    if (formData.shippingLastName) result.shipping.last_name = formData.shippingLastName;
    if (formData.shippingAddress1) result.shipping.address = formData.shippingAddress1;
    if (formData.shippingAddress2) result.shipping.address_2 = formData.shippingAddress2;
    if (formData.shippingCity) result.shipping.city = formData.shippingCity;
    if (formData.shippingRegion) result.shipping.region = formData.shippingRegion;
    if (formData.shippingPostal) result.shipping.postal = formData.shippingPostal;
    if (formData.shippingCountry) result.shipping.country = formData.shippingCountry;
  }
  
  // Credit card data
  if (formData.cardBin || formData.cardLast4 || formData.avsResult || formData.cvvResult) {
    result.credit_card = {};
    if (formData.cardBin) result.credit_card.issuer_id_number = formData.cardBin;
    if (formData.cardLast4) result.credit_card.last_4_digits = formData.cardLast4;
  ...
  if (formData.transactionAmount || formData.transactionCurrency || 
      formData.transactionId || formData.shopId || formData.transactionType) {
    ...
    if (formData.transactionCurrency) result.order.currency = formData.transactionCurrency;
    if (formData.transactionId) result.order.transaction_id = formData.transactionId;
    if (formData.shopId) result.order.shop_id = formData.shopId;
    if (formData.transactionType) result.order.transaction_type = formData.transactionType;
  }
  ...
  return {
    ...
    insights: {
      ...
      addressInfo: {
        ...
        shippingToBillingDistance: data.shipping_address?.distance_to_billing_address
      }
    },
  };
}