...
export async function GET(request: Request) {
  try {
    ...
    const highRiskTransactions = transactions.filter(t => t.riskScore >= 75).length;
    const mediumRiskTransactions = transactions.filter(t => t.riskScore >= 30 && t.riskScore < 75).length;
    const lowRiskTransactions = transactions.filter(t => t.riskScore < 30).length;
    ...
    return NextResponse.json(
      { error: 'Failed to retrieve dashboard data', details: error.message }, 
      { status: 500 }
    );
  }
}
...
function groupTransactionsByTimeUnit(transactions, timeframe) {
  const groupedData = {};
  
  // biome-ignore lint/complexity/noForEach: <explanation>
    transactions.forEach((transaction: { createdAt: string | number | Date; riskScore: any; }) => {
    let timeUnit: string;
    ...
    switch (timeframe) {
      ...
      case 'year':
        // Group by month
        timeUnit = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
        break;
      default:
        timeUnit = date.toISOString().split('T')[0];
    }
